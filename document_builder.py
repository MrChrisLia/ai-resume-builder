...
Templates: modern | classic | minimal | japanese
Formats:   .docx | .pdf | .md
"""
...
def _build_japanese_docx(data: dict, output_path: str):
    ...
    if stream:
        ...
        _set_font(tp.add_run('職務経歴書'), 20, bold=True, color=JBL, font_name='Yu Gothic')
        ...
        _set_font(title_p.add_run('職務経歴書'), 20, bold=True, color=JBL, font_name='Yu Gothic')
    ...
    if data.get('experience'):
        ...
        for exp in data['experience']:
            end = exp.get('end_date') or '現在'
            p = doc.add_paragraph()
            p.paragraph_format.space_before = Pt(7)
            _set_font(p.add_run(f"◆ {exp.get('company','')}"), 11, bold=True, color=JBL, font_name='Yu Gothic')
            p2 = doc.add_paragraph()
            p2.paragraph_format.space_after = Pt(2)
            _set_font(p2.add_run(f"　役職：{exp.get('title','')}　　期間：{exp.get('start_date','')} 〜 {end}"), 9, color=JGR, font_name='Yu Gothic')
            for bullet in exp.get('bullets', []):
                bp = doc.add_paragraph()
                bp.paragraph_format.left_indent = Inches(0.2)
                bp.paragraph_format.space_after  = Pt(1)
    ...
    if data.get('education'):
        ...
        for edu in data['education']:
            ...
            p.paragraph_format.space_after  = Pt(2)
            _set_font(p.add_run(f"{edu.get('school','')}　{deg}　{grad}卒"), 10, font_name='Yu Gothic')
...
def _render_html(data: dict, template: str = 'modern') -> str:
    ...
    if is_japanese:
        ...
        jp = JP_SECTIONS if 'JP_SECTIONS' in dir() else {
            'summary': '職務要約', 'experience': '職務経歴', 'education': '学歴',
            'skills': '保有スキル', 'languages': '語学力', 'projects': 'プロジェクト',
            'certifications': '資格・認定'
        }
    else:
        name_contact = f'''
    ...
    prefix = '■ ' if is_japanese else ''

    if data.get('summary'):
        sections.append(f'<div class="section-title">{prefix}{jp["summary"]}</div><p>{data["summary"]}</p>')

    if data.get('experience'):
        exp_html = ''
        for exp in data['experience']:
            end = exp.get('end_date') or ('現在' if is_japanese else 'Present')
            bullets = ''.join(f'<li>{b}</li>' for b in exp.get('bullets', []))
            if is_japanese:
                exp_html += f'''
                <div class="company">◆ {exp.get("company","")}</div>
                <div class="jp-role">役職：{exp.get("title","")}　期間：{exp.get("start_date","")} 〜 {end}</div>
                <ul>{bullets}</ul>'''
            else:
                exp_html += f'''
                <div class="entry">
        ...
        sections.append(f'<div class="section-title">{prefix}{jp["experience"]}</div>{exp_html}')
    ...
    if data.get('skills'):
        ...
        sections.append(f'<div class="section-title">{prefix}{jp["skills"]}</div>{skill_html}')
    ...
    if langs:
        if is_japanese:
            ...
            lang_html = ''.join(_jp_lang(l) for l in langs)
        else:
            lang_html = f'<p>{", ".join(_lang_str(l) for l in langs)}</p>'
        sections.append(f'<div class="section-title">{prefix}{jp["languages"]}</div>{lang_html}')
    ...
    if data.get('education'):
        ...
        for edu in data['education']:
            ...
            if edu.get('gpa'): deg += f' | GPA: {edu["gpa"]}'
            if is_japanese:
                edu_html += f'<p><strong>{edu.get("school","")}</strong>　{deg}　{edu.get("graduation","")}卒</p>'
            else:
                edu_html += f'''
                <div class="entry">
                  <div class="entry-header">
        ...
        sections.append(f'<div class="section-title">{prefix}{jp["education"]}</div>{edu_html}')
    ...
    if data.get('projects'):
        ...
        for proj in data['projects']:
            ...
            proj_html += '</div>'
        sections.append(f'<div class="section-title">{prefix}{jp["projects"]}</div>{proj_html}')

    certs = [c for c in data.get('certifications', []) if c]
    if certs:
        cert_html = ''.join(f'<li>{c}</li>' for c in certs)
        sections.append(f'<div class="section-title">{prefix}{jp["certifications"]}</div><ul>{cert_html}</ul>')
